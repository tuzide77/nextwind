...
class IndexController extends PwBaseController {
	...
	public function threadAction() {
		...
		if ($threadList->page <= 1) {
			$seoBo->setDefaultSeo($lang->getMessage('SEO:bbs.thread.run.title'), '', 
	...
	public function readAction() {
		...
		$seoBo = PwSeoBo::getInstance();
		$lang = Wind::getComponent('i18n');
		$threadDisplay->page <= 1 && $seoBo->setDefaultSeo($lang->getMessage('SEO:bbs.read.run.title'), '', 
		...
		if ($pwforum->foruminfo['password']) {
			if (!$this->loginUser->isExists()) {
				$this->forwardAction('u/login/run', 
					array('backurl' => WindUrlHelper::createUrl('bbs/cate/run', array('fid' => $pwforum->fid))));
