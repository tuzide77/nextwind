...
;(function(){
	...
	var authProfile = {
		...
		successHandle : function(data, form, btn){
			//认证提交成功回调
			if(type == 'email') {
				form.find('.pop_cont').hide().after('<div class="pop_cont J_auth_email_popel"><dl style="padding:20px;"><dt style="width:40px;"><span class="icon_middle_warning"></span></dt><dd class="f14">已经发送一封认证邮件到：<span class="s4">'+ data.data.email +'</span><br>点击邮箱中的认证链接访问即可通过验证。<br>如果长时间没有收到邮件，你可以<br><a href="" id="J_email_auth_trigger">修改邮箱</a> 或者 <a id="J_email_auth_resend" href="">重新发送邮件</a></dd></dl></div>');
				...
				$('#J_email_auth_trigger').on('click', function(e){
					e.preventDefault();

					$('a.J_auth_items[data-type=email]').click();
				});
